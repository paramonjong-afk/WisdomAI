...
const fixtureWorkPackage = fixture.workPackages.find((item) => item.project_id === matches[0].project.id)!
const linked = applyLocalProjectGate(existing, 'link_existing_project', { project_id: matches[0].project.id, project_name: matches[0].project.name, match_evidence: matches[0].evidence, work_package_id: fixtureWorkPackage.id, work_package_name: fixtureWorkPackage.name, work_package_code: fixtureWorkPackage.code }, '2026-08-25T04:00:00Z')
...
assert.equal(linked.candidate_data.work_package_id, fixtureWorkPackage.id, 'existing Project link must persist the selected work scope')
...
const page = readFileSync('src/pages/MasterDataCenter/index.tsx', 'utf8') + readFileSync('src/pages/MasterDataCenter/MasterDataReviewDrawer.tsx', 'utf8')
...
for (const token of ['save_master_data_project_gate_v3', 'project_work_packages', 'work_package_id', 'proposed_work_package_name', 'candidate_project_work_scope_linked', 'master_candidate_event_key_required', 'master_data_candidate_versions', 'master_data_audit', 'is_company_manager', "package.status='active'", "project.status='active'"]) assert.match(workScopeMigration, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
assert.doesNotMatch(workScopeMigration, /delete from public\.(master_data_candidates|financial_transactions|line_messages|document_flow_items|master_data_audit)/i)
