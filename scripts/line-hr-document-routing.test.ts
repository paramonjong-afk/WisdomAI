...
assert.match(telegram, /decodeJwtRole/)
assert.match(telegram, /role\?:unknown/)
assert.match(telegram, /employee_intakes\?select=id&limit=0/)
assert.match(telegram, /verification\?\.ok/)
assert.match(telegram, /if\(!await isAuthorizedServiceRequest\(request\)\)return json\(\{error:'Unauthorized'\},401\)/)
