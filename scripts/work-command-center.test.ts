...
assert.match(source,/view\s*===\s*["']doing["'][\s\S]*?actionableRows\.filter\(\(row\)\s*=>\s*hasActiveWorkerClaim\(row, claimNow\)\)/)
...
assert.match(source,/view\s*===\s*["']active["'][\s\S]*?actionableRows\.filter\(\(row\)\s*=>\s*row\.status\s*!==\s*["']done["']\)/)
...
assert.match(source,/heartbeat_at,lease_expires_at,approval_status,approval_fingerprint,attempt_count,worker_outcome,worker_outcome_reason,worker_outcome_at,requirement_version,controller_owner,execution_owner,qa_owner,control_state,checkpoint,context_manifest,new_information_hash,[^"\r\n]*created_at,updated_at/)
