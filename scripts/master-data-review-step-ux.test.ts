...
assert.equal(masterReviewBlockers(projectReady, 'ตรวจแล้ว', false).some((item) => item.includes('Version/Audit')), false, 'matching evidence must unlock summary without a redundant Correction')
...
const sourceReference = readFileSync('src/pages/MasterDataCenter/MasterDataSourceReferenceCard.tsx', 'utf8')
...
for (const token of ['MasterDataReviewDrawer', 'loadMasterDataReviewReceipts', 'validatePersistedReviewAction', 'validatePersistedCorrection', 'validatePersistedProjectGate', 'await load()']) assert.match(page, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
for (const token of ['MasterDataReviewProgress', 'MasterDataReviewActions', 'Raw/OCR', '1. ตรวจและเติมข้อมูล', '2. สรุปและยืนยัน', 'MasterDataSourceReferenceCard']) assert.match(drawer, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
...
console.log('master data review 2-tab UX passed: 53 fixture rows, 55+1 incident count reconciliation, Project/Work Scope → optional Correction → summary/confirm, RPC/refetch persistence guards and inline persisted status')
