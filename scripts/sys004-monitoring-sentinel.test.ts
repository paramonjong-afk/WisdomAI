...
const reconciliation = readFileSync(new URL('../supabase/migrations/202609160005_reconcile_sys004_resolved_children.sql', import.meta.url), 'utf8')
...
assert.match(hardening, /checkpoint='\{\}'::jsonb/)
assert.doesNotMatch(hardening, /checkpoint=null/)
...
assert.match(commandCenter, /ไม่มี Worker กำลังทำเพราะยังไม่มีงานที่อนุมัติพร้อม claim/)
assert.match(commandCenter, /มีงานอนุมัติแล้วรอ Worker/)
assert.match(reconciliation, /SYS-004-ORPHAN-CLAIM-ROOTCAUSE-001/)
assert.match(reconciliation, /SYS-004-RECOVERY-001/)
assert.match(reconciliation, /item\.worker_id is null/)
assert.match(reconciliation, /system_work_item_events/)
