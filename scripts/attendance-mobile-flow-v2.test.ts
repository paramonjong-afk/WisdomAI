...
for(const token of ["storage.from('attendance-selfies').remove",'storage_delete_not_verified','attendance_selfie_retention_events',"status:'failed'"]) assert.ok(retention.includes(token),`retention worker missing ${token}`)
