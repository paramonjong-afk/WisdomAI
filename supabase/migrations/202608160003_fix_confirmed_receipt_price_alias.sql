-- Remove PL/pgSQL record/SQL alias ambiguity in confirmed receipt price correction.
...
begin
  ...
  update public.vendor_product_prices price set
    quantity=gr_line.received_quantity,stated_unit_price=price_line.unit_price,effective_unit_price=price_line.unit_price,
    price_basis='actual',observed_at=coalesce(doc.document_date,doc.created_at::date),updated_at=now()
  from public.accounting_document_lines price_line
  join public.goods_receipt_line_reviews gr_line on gr_line.document_line_id=price_line.id and gr_line.accepted
