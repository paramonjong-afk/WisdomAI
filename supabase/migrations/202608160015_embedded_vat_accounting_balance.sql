-- Keep VAT metadata on the document, but do not debit VAT twice when OCR lines
-- already contain the gross amount (common on abbreviated tax invoices/receipts).
...
begin
  ...
  if vat_is_embedded then
    begin
      update public.accounting_documents set vat_amount=0 where id=p_document_id;
      perform public.confirm_accounting_document_pre_match(p_document_id);
    exception when others then
      update public.accounting_documents set vat_amount=original_vat where id=p_document_id;
      raise;
    end;
    update public.accounting_documents set vat_amount=original_vat where id=p_document_id;
  else
...
comment on function public.confirm_accounting_document(uuid) is
  'Confirms non-AP documents and prevents embedded VAT from being debited twice while retaining document VAT metadata.';
