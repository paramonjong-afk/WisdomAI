...
create unique index if not exists transfer_slip_money_allocations_active_key_idx
  on public.transfer_slip_money_allocations(lineage_id, allocation_key)
...
declare
  ...
  next_departments text[] := array[]::text[];
  next_destinations text[] := array[]::text[];
...
begin
  ...
  for allocation in select value from jsonb_array_elements(coalesce(target_allocations, '[]'::jsonb)) loop
    ...
    insert into public.transfer_slip_money_allocations(
      ...
      payee_name, responsible_name, description, confidence, evidence, status, version, created_by, confirmed_by, confirmed_at
    ) values (
      ...
      coalesce((select max(previous.version) + 1 from public.transfer_slip_money_allocations previous
                where previous.lineage_id = lineage_row.id and previous.allocation_key = allocation_key_value), 1),
