-- Use the canonical profiles.role column for service-role approvals and the
-- existing auth-bound is_platform_admin() helper for interactive approvals.
...
begin
  if auth.role()='service_role' then
    select coalesce(profile.role='admin',false) into actor_is_platform_admin
    from public.profiles as profile where profile.id=actor_profile_id;
  else
    if actor_profile_id<>auth.uid() then raise exception 'actor_mismatch'; end if;
    actor_is_platform_admin:=public.is_platform_admin();
  end if;
  if not coalesce(actor_is_platform_admin,false) then raise exception 'platform_admin_required'; end if;
