...
| v1.6 | 6/9/2569 | Close the remaining 601-768px and compact Chip/Tab touch-target gaps found by authenticated Production audit | shared theme only; clickable chips, tabs, buttons, icon buttons, inputs and dialogs use the mobile contract through 768px or coarse pointers; permissions and data scope are unchanged | None | responsive contract, typecheck, lint, build, 320/768/desktop viewport smoke and authenticated Production route audit | revert the shared theme and contract-test commit; no business data or migration is changed |
