...
| v1.4 | 17/9/2569 | แก้ Controller status ล้มจาก legacy service-role JWT `issued at future` | `automation-worker` ใช้ Supabase Secret API key รุ่นใหม่จาก `WISDOM_SUPABASE_SECRET_KEY` ก่อน และเก็บ legacy key เป็น fallback | automation claim contract, deploy function, authenticated status/claim-safe smoke | ลบ `WISDOM_SUPABASE_SECRET_KEY` เพื่อกลับไปใช้ legacy key โดยไม่เปลี่ยนคิวหรือ Audit |
