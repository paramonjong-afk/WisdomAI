...
| v2.2 | 6/9/2569 | Remove the Local reconciliation fixture and browser-storage save path from the Production Advance Detail page; the reconciliation step remains visible as a central Flow/Audit state | No migration; Production reads the existing central Advance/Document Flow/Audit records only | reconciliation fixture isolation, mobile contract, typecheck, lint, build, CI verification and authenticated Advance smoke | Revert the page-only change; retain central cases, source slips and Audit, and keep the fixture under test-only paths |
