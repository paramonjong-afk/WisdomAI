...
- The Detail Drawer exposes two tabs: `ตรวจและเติมข้อมูล` and `สรุปและยืนยัน`. Tab 1 links Project plus Work Package (or opens compact Project/Work Package entry), compares existing Master data with new evidence, and shows transfer sender/recipient pairs. It displays correction inputs only for missing or conflicting derived data. Tab 2 shows the final classification, destination, owner, next action, compact status/Audit and readable Source Reference before one-shot confirmation. Secondary request-information/reject/archive actions live in the additional-actions menu. A disabled Primary Action lists every missing field or prerequisite inline rather than relying on a snackbar.
