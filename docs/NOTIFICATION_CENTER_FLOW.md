...
flowchart LR
  ...
  E --> F[กรอง Filter + Module + Type]
  F --> G{ผู้ใช้เลือกการกระทำ}
  G -->|เปิดงาน| H[Deep link ไป Module ต้นทาง]
  G -->|อ่านแล้ว/อ่านทั้งหมด| I[notification_read_states\nเฉพาะผู้ใช้และ idempotent]
  I --> J[Mutation attempt / Audit\nอ่านทั้งหมดเฉพาะรายการในมุมมองปัจจุบัน]
  C -->|โหลดล้มเหลว| K[แสดง Error และให้ Refresh]
  I -->|บันทึกล้มเหลวบางรายการ| L[คงรายการที่ล้มเหลวและให้ Retry]
```
...
| Output | Snapshot สูงสุด 100 เหตุการณ์, จำนวนยังไม่อ่าน, จำนวนงานที่ต้องทำ, Priority, Owner, SLA, Deep link, Source Reference และตัวกรอง Type ที่เลือกได้ |
...
- Filter `all/unread/actionable/system`, Module และ Type เก็บใน URL เพื่อ refresh/back/forward ได้สอดคล้องกัน.
- `อ่านแล้วทั้งหมด` ทำเฉพาะรายการที่ยังไม่อ่านในมุมมองปัจจุบัน (แท็บ + Module + Type) ไม่ปิดงานต้นทางและไม่ลด `actionableCount`; แต่ละรายการใช้ notification key เดิมและ request idempotency เดิม.
...
- อ่านทั้งหมดบางรายการล้มเหลว: อัปเดตเฉพาะรายการที่บันทึกสำเร็จ คงรายการที่ล้มเหลวเป็นยังไม่อ่าน และแสดงจำนวนที่ต้อง Retry.
...
เหตุการณ์ต้นทางเก็บ audit ที่ Module เจ้าของ ส่วนการกดอ่านแล้ว/อ่านทั้งหมดผ่าน `runWithMutationAttempt` และใช้ `event_id` เป็น notification key. การตรวจต้องครอบคลุม tenant permission, unread/actionable count, URL filters (รวม Type), refresh, read idempotency, bulk partial failure/retry, deep link, Desktop/Tablet/Mobile และ console error.
...
| v1.3 | 29/8/2569 | ลดเวลาจัดการแจ้งเตือนจำนวนมากและแยกประเภทเหตุการณ์ให้ค้นหาได้ตรงจุด | เพิ่ม Type filter ใน URL และปุ่มอ่านทั้งหมดแบบ scoped current view พร้อม partial failure/retry และ idempotent request key | ไม่มี; ใช้ `notification_read_states` เดิม | Notification Center contract, typecheck, lint, build และ authenticated smoke ของ filter/mark-all | revert UI/filter/bulk handler; read state ที่บันทึกแล้วคงอยู่และไม่กระทบงานต้นทาง |
