...
type ClientActivityEvidence={event_id:string;event_type:string;severity:string;page_path:string|null;message:string|null;device_label:string|null;profile_id:string|null;reason:string|null;source:string|null;created_at:string}
type AuditAction=
...
const checkDiagnosis=(check:Check)=>{
  if(check.check_key==='user_side_events')return {
    cause:'เกิดจากการกระทำ เงื่อนไข หรือสิทธิ์ฝั่งผู้ใช้/อุปกรณ์ เช่น ยกเลิก เลือกไฟล์ไม่รองรับ หรือยังไม่มีสิทธิ์ใช้งาน',
    impact:'เก็บเป็นหลักฐานเพื่อช่วยเหลือผู้ใช้ แต่ไม่เปลี่ยนสถานะสุขภาพ ไม่เปิด Incident และไม่สร้างงานแก้ระบบกลาง',
    resolution:'ตรวจรายละเอียดรายเหตุการณ์ แล้วแนะนำผู้ใช้หรือแก้ข้อมูล/สิทธิ์เฉพาะรายตามสาเหตุ',
    confidence:'จำแนกจากประเภทเหตุการณ์และ reason ที่บันทึกไว้',
  }
  if(check.check_key==='client_errors')return {
    cause:check.status==='critical'?'พบข้อผิดพลาดของโปรแกรมฝั่งเว็บที่กระทบการทำงานซ้ำหลายครั้ง':'พบข้อผิดพลาดของโปรแกรมฝั่งเว็บที่ควรตรวจสอบ',
    impact:'อาจกระทบผู้ใช้หลายรายและถือเป็นปัญหาของระบบกลาง จึงนำไปคำนวณสุขภาพและ Incident',
    resolution:'ตรวจเหตุการณ์ หน้า ข้อความ และแหล่งที่มา แก้ระบบ แล้วกดตรวจใหม่เพื่อยืนยันการฟื้นตัว',
    confidence:'จำแนกจาก telemetry ที่ระบุความรับผิดชอบฝั่งระบบ',
  }
  ...
  return <Stack spacing={3}>
    ...
    <Drawer anchor="right" open={Boolean(selectedCheck)} onClose={()=>setSelectedCheck(null)} slotProps={{paper:{sx:{width:{xs:'100%',sm:480},p:3}}}}>
      {selectedCheck&&<Stack spacing={2}>
        ...
        <Divider/><Stack direction="row" spacing={1} useFlexGap sx={{flexWrap:'wrap'}}><Chip color={statusColor[selectedCheck.status]} label={statusLabel[selectedCheck.status]}/><Chip variant="outlined" label={selectedCheck.module}/>{selectedCheck.check_key==='user_side_events'&&<Chip color="info" variant="outlined" label="ข้อมูลฝั่งผู้ใช้ · ไม่กระทบระบบกลาง"/>}{selectedCheck.check_key==='client_errors'&&<Chip color="error" variant="outlined" label="ข้อผิดพลาดฝั่งระบบ"/>}</Stack>
        ...
        {(selectedCheck.status!=='healthy'||selectedCheck.check_key==='user_side_events')&&<Stack spacing={1.25}>
        ...
        {['client_errors','user_side_events'].includes(selectedCheck.check_key)&&Array.isArray(selectedCheck.metadata?.events)?<Box><Typography variant="subtitle2" sx={{mb:1}}>หลักฐานรายเหตุการณ์</Typography><Stack spacing={1.25}>{(selectedCheck.metadata.events as ClientActivityEvidence[]).map(event=><Paper key={event.event_id} variant="outlined" sx={{p:1.5}}><Stack spacing={.75}><Stack direction="row" spacing={1} useFlexGap sx={{justifyContent:'space-between',flexWrap:'wrap'}}><Typography sx={{fontWeight:800}}>{event.message||event.event_type}</Typography><Chip size="small" variant="outlined" label={event.reason||event.event_type}/></Stack><Typography variant="body2">หน้า {event.page_path||'-'} · {event.device_label||'ไม่ระบุอุปกรณ์'}</Typography><Typography variant="body2" color="text.secondary">{formatDate(event.created_at)} · ผู้ใช้ {event.profile_id||'ไม่ระบุ'} · แหล่ง {event.source||'-'}</Typography></Stack></Paper>)}</Stack></Box>:selectedCheck.check_key==='attendance'&&Array.isArray(selectedCheck.metadata?.stale_sessions)&&selectedCheck.metadata.stale_sessions.length>0?<Box><Typography variant="subtitle2" sx={{mb:1}}>รายการที่ต้องตรวจสอบ</Typography><Stack spacing={1.25}>{(selectedCheck.metadata.stale_sessions as StaleAttendanceSession[]).map(session=><Paper key={session.session_id} variant="outlined" sx={{p:1.5}}><Stack spacing={.75}><Stack direction="row" sx={{justifyContent:'space-between',gap:1}}><Typography sx={{fontWeight:800}}>{session.employee_name||'ไม่พบชื่อพนักงาน'}</Typography><Chip size="small" color="warning" label={`ค้าง ${formatDuration(session.open_minutes)}`}/></Stack><Typography variant="body2">{session.employee_code?`รหัส ${session.employee_code} · `:''}{session.company_name||'ไม่พบชื่อบริษัท'}</Typography><Typography variant="body2">{session.project_name||'ไม่พบโครงการ'} · {session.site_name||'ไม่พบไซต์'}</Typography><Typography variant="body2" color="text.secondary">ลงเวลาเข้า {formatDate(session.clock_in_at)}</Typography><Button variant="contained" size="small" href={`/reports?month=${session.clock_in_at.slice(0,7)}&employee=${session.profile_id}&session=${session.session_id}`}>เปิดและตรวจสอบรายการนี้</Button></Stack></Paper>)}</Stack></Box>:<Box><Typography variant="caption" color="text.secondary">ข้อมูลประกอบ</Typography>{selectedCheck.metadata&&Object.keys(selectedCheck.metadata).length?<Stack spacing={1} sx={{mt:1}}>{Object.entries(selectedCheck.metadata).filter(([key])=>key!=='stale_sessions').map(([key,value])=><Paper key={key} variant="outlined" sx={{p:1.25}}><Typography variant="caption" color="text.secondary">{key}</Typography><Typography sx={{wordBreak:'break-word'}}>{typeof value==='object'?JSON.stringify(value):String(value)}</Typography></Paper>)}</Stack>:<Typography>ไม่มีข้อมูลเพิ่มเติม</Typography>}</Box>}
