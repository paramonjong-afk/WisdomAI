...
type StaleAttendanceSession={session_id:string;company_id:string;company_name:string|null;profile_id:string;employee_name:string|null;employee_code:string|null;project_name:string|null;site_name:string|null;clock_in_at:string;open_minutes:number;overdue_minutes:number;status:string}
type AuditAction=
  ...
  return <Stack spacing={3}>
    ...
    <Drawer anchor="right" open={Boolean(selectedCheck)} onClose={()=>setSelectedCheck(null)} slotProps={{paper:{sx:{width:{xs:'100%',sm:480},p:3}}}}>
      {selectedCheck&&<Stack spacing={2}>
        ...
        {selectedCheck.check_key==='attendance'&&<Paper variant="outlined" sx={{p:1.5,bgcolor:'success.50'}}><Typography variant="subtitle2">กำลังปฏิบัติงานวันนี้</Typography><Typography variant="body2" color="text.secondary">เข้างานแล้วแต่ยังไม่ถึงเกณฑ์แจ้งเตือน ไม่ถือเป็นปัญหาระบบ</Typography><Typography sx={{fontWeight:800,fontSize:24}}>{Number(selectedCheck.metadata?.active_sessions_today??0).toLocaleString('th-TH')} รายการ</Typography></Paper>}
        {selectedCheck.check_key==='attendance'&&Array.isArray(selectedCheck.metadata?.stale_sessions)&&selectedCheck.metadata.stale_sessions.length>0?<Box><Typography variant="subtitle2" sx={{mb:1}}>เกินเวลาตรวจสอบ</Typography><Stack spacing={1.25}>{(selectedCheck.metadata.stale_sessions as StaleAttendanceSession[]).map(session=><Paper key={session.session_id} variant="outlined" sx={{p:1.5}}><Stack spacing={.75}><Stack direction="row" sx={{justifyContent:'space-between',gap:1}}><Typography sx={{fontWeight:800}}>{session.employee_name||'ไม่พบชื่อพนักงาน'}</Typography><Chip size="small" color="warning" label={`เกิน ${formatDuration(session.overdue_minutes)}`}/></Stack><Typography variant="body2">{session.employee_code?`รหัส ${session.employee_code} · `:''}{session.company_name||'ไม่พบชื่อบริษัท'}</Typography><Typography variant="body2">{session.project_name||'ไม่พบโครงการ'} · {session.site_name||'ไม่พบไซต์'}</Typography><Typography variant="body2" color="text.secondary">ลงเวลาเข้า {formatDate(session.clock_in_at)}</Typography><Button variant="contained" size="small" href={`/reports?month=${session.clock_in_at.slice(0,7)}&employee=${session.profile_id}&session=${session.session_id}`}>เปิดและตรวจสอบรายการนี้</Button></Stack></Paper>)}</Stack></Box>:<Box><Typography variant="caption" color="text.secondary">ข้อมูลประกอบ</Typography>{selectedCheck.metadata&&Object.keys(selectedCheck.metadata).length?<Stack spacing={1} sx={{mt:1}}>{Object.entries(selectedCheck.metadata).filter(([key])=>key!=='stale_sessions').map(([key,value])=><Paper key={key} variant="outlined" sx={{p:1.25}}><Typography variant="caption" color="text.secondary">{key}</Typography><Typography sx={{wordBreak:'break-word'}}>{typeof value==='object'?JSON.stringify(value):String(value)}</Typography></Paper>)}</Stack>:<Typography>ไม่มีข้อมูลเพิ่มเติม</Typography>}</Box>}
