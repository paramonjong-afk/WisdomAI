...
type DailyDeliveryRow = {
  ...
  sent_at: string | null
  read_at: string | null
  daily_wage_transfer_confirmations: { transfer_date: string; amount: number } | null
...
export function AdvancePaymentReportPage() {
  ...
  const [dailyDeliveries, setDailyDeliveries] = useState<DailyDeliveryRow[]>([])

  const load = useCallback(async () => {
    if (!companyId || !canManage) return
    setLoading(true); setError('')
    const [result, dailyResult] = await Promise.all([
      advanceReportGateway.list(companyId, from, to, undefined, status || undefined),
      supabase.from('daily_wage_transfer_deliveries')
        .select('id,employee_profile_id,status,sent_at,read_at,daily_wage_transfer_confirmations(transfer_date,amount),profiles!daily_wage_transfer_deliveries_employee_profile_id_fkey(full_name)')
    ...
    if (dailyResult.error) setError((current) => current || userError(dailyResult.error))
    else setDailyDeliveries((dailyResult.data ?? []) as unknown as DailyDeliveryRow[])
    setLoading(false)
  ...
  return <Stack spacing={2.5}>
    ...
    {dailyDeliveries.length > 0 && <Paper variant="outlined" sx={{ p: 2 }}><Typography variant="h6" sx={{ fontWeight: 800, mb: 1 }}>ค่าแรงรายวัน · Web Chat Delivery</Typography><Table size="small"><TableHead><TableRow><TableCell>พนักงาน</TableCell><TableCell>วันที่โอน</TableCell><TableCell align="right">ยอด</TableCell><TableCell>สถานะส่ง</TableCell><TableCell>เวลา</TableCell></TableRow></TableHead><TableBody>{dailyDeliveries.map((row) => <TableRow key={row.id}><TableCell>{row.profiles?.full_name ?? row.employee_profile_id}</TableCell><TableCell>{row.daily_wage_transfer_confirmations?.transfer_date ?? '-'}</TableCell><TableCell align="right">{money(row.daily_wage_transfer_confirmations?.amount)}</TableCell><TableCell><Chip size="small" color={row.status === 'read' ? 'success' : row.status === 'sent' ? 'info' : row.status === 'failed' ? 'error' : 'warning'} label={row.status === 'read' ? 'อ่านแล้ว' : row.status === 'sent' ? 'ส่งแล้ว' : row.status === 'failed' ? 'ส่งไม่สำเร็จ' : 'รอส่ง'} /></TableCell><TableCell>{dateText(row.read_at ?? row.sent_at)}</TableCell></TableRow>)}</TableBody></Table></Paper>}
