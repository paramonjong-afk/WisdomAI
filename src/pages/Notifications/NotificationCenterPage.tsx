...
export function NotificationCenterPage() {
  ...
  const load = async () => { if (!user?.id && !localFixture) return; setLoading(true); setError(''); try { setSnapshot(await loadNotificationSnapshot({ companyId, profileId, localFixture })) } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'โหลดแจ้งเตือนไม่สำเร็จ') } finally { setLoading(false) } }
  useEffect(() => { const refresh = async () => { if (!profileId && !localFixture) return; setLoading(true); try { setSnapshot(await loadNotificationSnapshot({ companyId, profileId, localFixture })) } catch { setError('โหลดแจ้งเตือนไม่สำเร็จ') } finally { setLoading(false) } }; const initial = window.setTimeout(() => void refresh(), 0); const timer = window.setInterval(() => void refresh(), 30_000); return () => { window.clearTimeout(initial); window.clearInterval(timer) } }, [companyId, localFixture, profileId])
  ...
  if (!profile && !localFixture) return <Alert severity="info">กรุณาเข้าสู่ระบบเพื่อดูการแจ้งเตือน</Alert>
