...
function flowNodes(row: AdvanceCase): FlowNode[] {
  ...
  const remarkNode: FlowNode = { key: 'reconciliation', label: 'Remark กระทบยอดเงินเข้า', status: row.status === 'closed' ? 'passed' : 'missing', time: closeAudit?.created_at ?? null, owner: row.status === 'closed' ? 'ผู้อนุมัติ / จาก Audit กลาง' : 'Flow กลาง / ยังไม่ระบุ', documentId, audit: audits, detail: row.status === 'closed' ? 'ปิดยอดแล้วตามสถานะและ Audit กลาง' : 'ต้องบันทึกผ่าน Flow กลางของรายการ ไม่ใช้ข้อมูลในเครื่อง' }
  return [
