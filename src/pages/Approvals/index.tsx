...
type AttendanceException={id:string;action:string;requested_at:string;distance_meters:number;accuracy_meters:number;profiles:{full_name:string|null;email:string|null}|null;project_sites:{name:string;projects:{name:string}|null}|null}
...
export function ApprovalsPage(){
  ...
  const load=useCallback(async()=>{
    ...
    const [w,ledger,l,c,cl,a,exceptions]=await Promise.all([
      ...
      supabase.from('attendance_channel_requests').select('id,action,requested_at,distance_meters,accuracy_meters,profiles(full_name,email),project_sites(name,projects(name))').eq('request_kind','location_exception').in('status',['pending_review','claimed','information_required']).order('requested_at').limit(200),
    ])
  ...
  const reviewException=(id:string,action:'approve'|'reject'|'request_more')=>run(()=>supabase.rpc('review_attendance_location_exception',{target_request_id:id,review_action:action,review_reason:reviewNote.trim()}),action==='approve'?'อนุมัติคำขอนอกพื้นที่แล้ว':action==='reject'?'ปฏิเสธคำขอนอกพื้นที่แล้ว':'ขอข้อมูลเพิ่มแล้ว')
