...
export function MasterDataCenterPage() {
  ...
  const correctCandidate = async () => {
    ...
    if (localTestData) {
      const updated = { ...selected, display_name: correction.display_name.trim() || selected.display_name, classification_type: correction.classification_type, status: 'admin_reviewed', review_reason: reviewReason.trim(), reviewed_at: new Date().toISOString(), candidate_data: { ...selected.candidate_data, ...correction, local_correction_version: Number(selected.candidate_data.local_correction_version ?? 0) + 1 } }
      setSelected(updated); setCandidates((current) => current.map((row) => row.id === updated.id ? updated : row)); setDrawerMessage({ severity: 'success', text: 'Local fixture: append ฉบับแก้ไขแล้ว รายการยังคงอยู่ในคิวจนกว่าจะยืนยัน' }); return
    }
    setSavingId(selected.id); setDrawerMessage(null); setError('')
    const { data: correctedData, error: correctionError } = await supabase.rpc('correct_master_data_candidate', { target_candidate_id: selected.id, target_event_key: crypto.randomUUID(), target_correction: correction, target_reason: reviewReason.trim() })
    setSavingId('')
    if (correctionError) { setDrawerMessage({ severity: 'error', text: userError(correctionError) }); return }
    if (correctedData) { const updated = correctedData as Candidate; setSelected(updated); setCandidates((current) => current.map((row) => row.id === updated.id ? updated : row)) }
  ...
  const saveProjectGate = async (action: ProjectGateAction, payload: Record<string, unknown>) => {
    ...
    if (localTestData) {
      const updated = applyLocalProjectGate(selected, action, payload)
    ...
    setSavingId(selected.id); setDrawerMessage(null); setError('')
    const { data, error: gateError } = await supabase.rpc('save_master_data_project_gate', { target_candidate_id: selected.id, target_event_key: crypto.randomUUID(), target_action: action, target_payload: payload, target_reason: reviewReason.trim() })
    setSavingId('')
    if (gateError) { setDrawerMessage({ severity: 'error', text: userError(gateError) }); return }
    const updated = data && typeof data === 'object' && 'candidate' in data ? (data as { candidate: Candidate }).candidate : null
    if (updated) { setSelected(updated); setCandidates((current) => current.map((row) => row.id === updated.id ? updated : row)) }
