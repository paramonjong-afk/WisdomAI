...
export function RoleRoute({access,children}:{access:Access;children:ReactNode}){
  ...
  const manager=profile?.role==='admin'||profile?.role==='manager'||['company_admin','executive','manager'].includes(companyRole??'')
  const admin=profile?.role==='admin'||companyRole==='company_admin'
  const platform=profile?.platform_role==='admin'
