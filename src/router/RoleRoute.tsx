...
export function RoleRoute({access,children}:{access:Access;children:ReactNode}){
  ...
  if(denied&&!localTestMode)return <Stack spacing={2} sx={{maxWidth:560,mx:'auto',mt:8}}><Alert severity="error">บัญชีนี้ไม่มีสิทธิ์เปิดหน้านี้</Alert><Button variant="contained" onClick={()=>navigate('/time-tracking',{replace:true})}>กลับหน้าลงเวลา</Button></Stack>
  return children
}
